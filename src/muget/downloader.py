...
def apply_metadata_ffmpeg(audio_file, metadata, cover_file=None):
    ...
    try:
        ...
        # Temporary files
        temp_output = TEMP_DIR / f"{audio_file.stem}.temp{audio_file.suffix}"
        metadata_file = None
        ...
        # Write comment using mutagen
        if is_opus:
            write_opus_metadata(audio_file, metadata)
            
        if not is_opus:
            write_m4a_metadata(audio_file, metadata)
...
def write_opus_metadata(audio_file, metadata):
    audio = OggOpus(audio_file)
    audio["comment"] = [metadata["comment"]]
    audio["encoder"] = [f"MuGet v{__version__}"]
    audio.save()

def write_m4a_metadata(audio_file, metadata):
    audio = MP4(audio_file)
    audio["\xa9too"] = [f"MuGet v{__version__}"]
    audio.save()
...
class YouTubeMusicDownloader:
    ...
    def download_song_with_metadata(self, song, metadata):
        ...
        temp_final = self.temp_dir / f"{track_str} - {title}.{extension}"
        ...
        try:
            ...
            po_token = self.config.get("po_token")

            if cookies_path:
                cmd_download += ["--cookies", cookies_path]

            extractor_args = []

            if po_token:
                extractor_args.append(f"po_token=web_music.gvs+{po_token}")
            else:
                extractor_args.append("formats=missing_pot")

            cmd_download += [
                "--extractor-args",
                "youtube:" + ";".join(extractor_args)
            ]
            ...
            if self.apply_metadata_and_cover(temp_final, metadata, cover_path):
            ...
            shutil.move(temp_final, final_file)
    ...
    def process_playlist(self):
        ...
        track_width = len(str(len(tracks)))
        ...
        for idx, track in enumerate(tracks, 1):
            if track.get('videoType') != 'MUSIC_VIDEO_TYPE_ATV':
                logger.warning(f"({idx:>{track_width}}/{len(tracks)}) {track['title']} skipped (not an official song)")
                skipped += 1
                continue
            ...
            if result == "skipped":
                skipped += 1  
                logger.info(f"(Track {idx:>{track_width}}/{len(tracks)}) Skipped \"{track['title']}\" (already exists)")
                
            elif result is True:     
                successful += 1                
                logger.info(f"(Track {idx:>{track_width}}/{len(tracks)}) Downloading \"{track['title']}\"")
            else:
                failed += 1
                logger.error(f"(Track {idx:>{track_width}}/{len(tracks)}) Failed: \"{track['title']}\"")
        ...
        if self.config.get("replaygain"):
            ...
            try:
                subprocess.run(
                    ["rsgain", "easy", str(self.output_dir)], check=True)
                logger.info("ReplayGain written")
